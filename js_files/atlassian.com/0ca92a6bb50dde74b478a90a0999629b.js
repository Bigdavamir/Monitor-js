...
;
try {
/* module-key = 'com.atlassian.support:conf-tags', location = 'includes/js/conf-tags.js' */
if(typeof window.ace=="undefined"){document.write('<script type="text/javascript" src="https://ace-cdn.atlassian.com/stp/current/analytics/js/atl-analytics.min.js"><\/script>');document.write("<script type=\"text/javascript\">ace.analytics.Initializer.initWithPageAnalytics('9nfqb81km3');<\/script>")}if(typeof window.optly=="undefined"){document.write('<script type="text/javascript" src="https://cdn.optimizely.com/js/1053703113.js"><\/script>')}AJS.toInit(function(a){a("#quick-search").submit(function(){analytics.track("search_submit_quicksearch",{category:"search",action:"submitted",label:"nav search field_"+a("#quick-search-query").val()})});a("#quicksearch").submit(function(){analytics.track("search_submit_quicksearch",{category:"search",action:"submitted",label:"nav search field_"+a("#search-input").val()})});a(document).on("click",".quick-search-dropdown ol li a",function(){var c=a(this).attr("href");var b=a(".quick-search-dropdown ol li").index(a(this).parent())+1;analytics.track("search_click_quicksearch_typeahead",{category:"search",action:"clicked",position:b,label:"nav search field_typeahead_"+c+"_"+b})});if(a("#searchPageForm").length>0){a("#searchPageForm").submit(function(){analytics.track("search_submit",{category:"search",action:"submitted",label:"results search field_"+a("#searchPageInput").val()})});a("#searchMenuContainer ul.#facetListContainer").on("click","li a",function(b){var c=a(this).data("filter");if(window.analytics){analytics.track("search_filter",{category:"search",action:"filtered",label:c})}});a("#searchMenuContainer").on("change","#productVersion",function(){var c=a("#searchMenuContainer #productVersion").val(),b=a("#searchMenuContainer #productId").val();analytics.track("search_filter",{category:"search",action:"filtered",product:b,label:"space_"+c})});a("#searchResultsContainer").on("click","a.results-link",function(){var c=a(this).attr("href"),b=a("#searchResultsContainer a").index(this)+1;analytics.track("search_click_result",{category:"search",action:"clicked",label:c+"_"+b})});a("#atlassianSearch #loadResultsButton").on("click",function(d){var c=a("#searchResultsContainer .results-link").length-30,b=Math.floor(c/10)+1;analytics.track("search_click_pagination",{category:"search",action:"clicked",label:"view_more_"+b})})}if(a("#search-form").length>0){a("#search-form").submit(function(){analytics.track("search_submit",{category:"search",action:"submitted",label:"results search field_"+a("#query-string").val()})});a("#filter-form").on("click",".aui-nav a",function(d){var b=a(this).closest("ul").data("key");var c=a(this).data("value");analytics.track("search_filter",{category:"search",action:"filtered",label:b+"_"+c})});a("#filter-form").on("change","#search-filter-by-space",function(){analytics.track("search_filter",{category:"search",action:"filtered",label:"space_"+a(this).val()})});a("#filter-form").on("change","input[type=checkbox]",function(){analytics.track("search_filter",{category:"search",action:"filtered",label:a(this).attr("name")+"_"+(a(this).attr("checked")?"true":"false")})});a("#filter-form").on("selected.autocomplete-user","#autocomplete-user",function(){analytics.track("search_filter",{category:"search",action:"filtered",label:"author_"+Confluence.Search.Util.getFormParams(a("#filter-form"))["contributorUsername"]})});a("#filter-form").on("click",".clear-button",function(){analytics.track("search_filter",{category:"search",action:"filtered",label:"author_none"})});a(document).on("click",".search-results-container ol.search-results li h3 a.search-result-link",function(){var c=a(this).attr("href");var b=a(".search-results-container ol.search-results li").index(a(this).closest("li"))+1;var d=Confluence.Search.Util.getQueryStringParams(window.location.search);if(d.startIndex){b+=parseInt(d.startIndex)}analytics.track("search_click_result",{category:"search",action:"clicked",position:b,label:c+"_"+b})});a(document).on("click",".pagination-container ul.pagination li a",function(){var b;if(a(this).hasClass("pagination-next")){b=parseInt(a(".pagination-curr").text())+1}else{if(a(this).hasClass("pagination-prev")){b=parseInt(a(".pagination-curr").text())-1}else{b=a(this).text()}}analytics.track("search_click_pagination",{category:"search",action:"clicked",label:"pagination_"+b})})}});
}catch(e){WRMCB(e)};
;
try {
