define('confluence/property-panel', [
    ...
    var PropertyPanel = {
            ...
            for (var i = 0, ii = buttons.length; i < ii; i++) {
                ...
                if (!button.isFooter) {
                    ...
                    if (!options.footer) {
                        ...
                        options.footer = $('<div></div>').attr({ class: 'panel-buttons panel-buttons-footer' });
                    }
                    options.footer.append(element);
                }
            }
        ...
        create: function(type, anchor, content, options) {
            ...
            parent = $('<div role="dialog" tabindex="0"></div>').addClass('aui-property-panel-parent').addClass(type + '-panel aui-box-shadow').attr('id', 'property-panel')
                .appendTo('body');
            ...
            $('#wysiwygTextarea_ifr').contents().on('keydown', this.tabHandler);
            this.trapFocus();
        ...
        destroy: function() {
            ...
            this.current = null;
            $('#wysiwygTextarea_ifr').contents().off('keydown', this.tabHandler);
        },
        /**
        ...
        /**
         * Handles tab key press to focus on the property panel.
        ...
        tabHandler: function(e) {
            ...
            if (e.key === 'Tab' && !e.shiftKey) {
                panel = $(document).find('.aui-property-panel').parent().get(0);
                if (!$.contains(panel, e.target)) {
                    panel.focus();
                }
            }
        },
         ...
         */
        trapFocus: function() {
            var focusableElements = $(document)
                .find('.aui-property-panel')
